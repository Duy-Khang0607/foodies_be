...
// VNPay configuration
const vnpayConfig = {
  tmnCode: "2OEQHL4T", // Terminal ID / Mã Website
  secureSecret: "2JOQ01SGQHJ7SHSE41WV2BZTQKT6G4AP", // Secret Key
  vnpayHost: "https://sandbox.vnpayment.vn", // URL thanh toán TEST
  testMode: true,
  hashAlgorithm: "SHA512", // VNPay yêu cầu SHA512
  loggerFn: ignoreLogger,
};

// Helper function để format date theo chuẩn VNPay
...
router.get("/check-payment", async (req, res) => {
  try {
    ...
    if (isValid) {
      // Convert amount from cents back to VND
      ...
      res.json({
        ...
        data: {
          ...
          amount: amountInVND, // Amount in VND
          amountInCents: amountInCents, // Original amount from VNPay
